...
def get_input_files(corpus_raw, ratio):
    ...
    for book_filename in book_filenames:
        ...
        book_file.close
        summ_it(corpus_raw, ratio, book_filename)
...
def write_summary(summary, book_filename):
    with open("/home/rachit/Music/summ/" + os.path.basename(book_filename).split('.')[0] + ".txt", "w+") as book_new_file:
		book_new_file.write(summary)
		book_new_file.close
    # return raw_data
        # print("Corpus is now {0} characters long".format(len(corpus_raw)))
    # return corpus_raw
...
def summ_it(raw_data, ratio, book_filename):
	...
	write_summary(summary, book_filename)
...
# Sentence extraction
raw_data = get_input_files(corpus_raw, ratio)